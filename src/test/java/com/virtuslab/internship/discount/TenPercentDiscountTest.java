...
class TenPercentDiscountTest {
    ...
    void shouldApply10PercentDiscountWhenPriceIsAbove50() {
        ...
        var expectedTotalPrice = cheese.price().add(steak.price()).multiply(BigDecimal.valueOf(0.9)).setScale(2, RoundingMode.HALF_UP);
    ...
    void shouldNotApply10PercentDiscountWhenPriceIsBelow50() {
        ...
        var receipt = new Receipt(receiptEntries);
        var discount = new TenPercentDiscount();
        var expectedTotalPrice = cheese.price().multiply(BigDecimal.valueOf(2)).setScale(2, RoundingMode.HALF_UP);
